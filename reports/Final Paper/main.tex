...
\begin{enumerate}
    \item Identifiers and Basic Information:
          \begin{itemize}
              ...
              \item Weekly Log Return: $\log\left(\frac{P_t}{P_{t-5}}\right)$, where $P_{t-5}$ is the adjusted closing price 5 trading days before time $t$ (since stocks are not traded on weekends).
                    % Explicar más de cómo obtenemos la label en la siguiente sección
              \item Class label: a categorical variable indicating the direction of the stock price movement (up, neutral, down), defined in terms of the sign of the Weekly Log Return.
          \end{itemize}
    \item Volume:
          \begin{itemize}
              \item Daily trading volume
              \item Normalized trading volume: the daily trading volume normalized by the average trading volume of the stock over the entire period ($\frac{V_t - \mu_V}{\sigma_V}$, where $V_t$ is the trading volume at time $t$, $\mu_V$ is the average trading volume, and $\sigma_V$ is the standard deviation of the trading volume).
          \end{itemize}
    \item Technical Indicators:
          \begin{itemize}
              ...
              \item Relative Strength Index (RSI): a momentum oscillator that measures the speed and change of price movements, calculated as $RSI = 100 - \frac{100}{1 + RS}$, where $RS$ is the average gain of up periods divided by the average loss of down periods over a specified time frame (10 trading days in our case).
...
\begin{itemize}
    \item Shared market sector AND high correlation between stock returns
    \item Shared market sector AND low divergence between the distributions of stock returns (measured using the Jensen-Shannon divergence)
\end{itemize}
...
\begin{itemize}
    ...
    \item For each stock, we obtained the data mentioned in the previous section, computing the necessary features. As to the labels, we decided to define a threshold $\epsilon$ on the Weekly Log Return to determine the class labels, in order to avoid labeling small fluctuations as significant movements. Specifically, we defined the class labels as follows:
          \begin{itemize}
              \item Up: if the Weekly Log Return is greater than $\epsilon$
              \item Neutral: if the Weekly Log Return is between $-\epsilon$ and $\epsilon$
              \item Down: if the Weekly Log Return is less than $-\epsilon$
          \end{itemize}
    ...
    \item We split the data into training, validation, and test sets, using a chronological split to avoid data leakage. Specifically, we use ratios of 60\%, 20\%, and 20\% for the training, validation, and test sets, respectively. Though this may be a somewhat aggressive split (keeping only 60\% of the data for training), it allows the splits to reflect similar time frames (dividing the dataset into 3, 1, and 1 years, respectively) and, thus, to be more comparable between them.

          This is done in different ways for the different models, though. For the Random Forest, we simply split the data according to the specified ratios and create a catabolic variable in the tabular dataset to indicate the split to which each snapshot belongs. For the LSTM, we create sequences of 20 days for each stock and split these sequences according to the specified ratios. For the GNN, we create the graph structure using the data from the training set, and then split the snapshots of the graph (this is, every node and edge at a given time step).
\end{itemize}
